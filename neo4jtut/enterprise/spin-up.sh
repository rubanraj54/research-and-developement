...
for (( i = 1; i <= $DEFAULT_NUMBER_OF_ROBOTS; i++ )); do
  INITIAL_HOSTS="$INITIAL_HOSTS instance$i:$COORDINATION_INSTANCE_PORT,"
done

for ((i=1;i<=$DEFAULT_NUMBER_OF_ROBOTS;i++));
do
  echo $DEFAULT_NUMBER_OF_ROBOTS >> $DOCKER_FILE_NAME
  echo -e "  instance$i:" >> $DOCKER_FILE_NAME
  ...
  echo -e "    hostname: instance$i" >> $DOCKER_FILE_NAME
  ...
  echo -e "      - NEO4J_ha_host_coordination=instance$i:$COORDINATION_INSTANCE_PORT" >> $DOCKER_FILE_NAME
  echo -e "      - NEO4J_ha_host_data=instance$i:$DATA_PORT" >> $DOCKER_FILE_NAME
  echo -e "      - NEO4J_ha_initialHosts=$INITIAL_HOSTS" >> $DOCKER_FILE_NAME
  if (( $i > 1 )); then
    echo -e "    depends_on:" >> $DOCKER_FILE_NAME
    echo -e "      - instance$((i-1))" >> $DOCKER_FILE_NAME
  fi
done
